...
impl Navigator {
    ...
    pub fn get_current_page(&self) -> Option<&Box<dyn Page>> {
        match self.pages.is_empty() {
            true => None,
            false => Some(&self.pages[self.pages.len() - 1]),
        }
    }
    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::UpdateEpicStatus { epic_id } => {
                self.db.update_epic_status(
                    epic_id,
                    (self.prompts.update_status)()
                        .ok_or(anyhow!("User provided the wrong argument."))?,
                )?;
            }
